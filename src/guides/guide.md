# Руководство по пользовательским сервисам

## 📋 Общая информация

Ваши сервисы настроены для автоматической работы. Основные компоненты:

1. **Code Server** - Веб-версия VS Code
2. **Nginx Proxy** - Прокси для доступа к Code Server

## 🚀 Быстрый старт

### Доступ к Code Server:
1. Откройте браузер
2. Перейдите по адресу: `http://сервер:ПОРТ`
   - Порт можно узнать: `user-service-control status`
   - Или посмотреть в файле: `cat ~/.code-server-initial-password.txt` (если есть)

### Первый вход:
- Используйте пароль из файла `~/.code-server-initial-password.txt`
- Рекомендуется сменить пароль сразу после входа

## 🛠 Управление сервисами

### Основные команды:

```bash
# Показать статус всех сервисов
user-service-control status

# Запустить все сервисы
user-service-control start

# Остановить все сервисы  
user-service-control stop

# Перезапустить все сервисы
user-service-control restart

# Показать список сервисов
user-service-control list

# Показать логи
user-service-control logs
```

### Управление отдельными сервисами:

```bash
# Только Code Server
user-service-control start code-server
user-service-control stop code-server
user-service-control status code-server

# Только Nginx
user-service-control start nginx-proxy
user-service-control stop nginx-proxy
```

## Смена пароля Code Server
```bash
# Интерактивная смена пароля
user-code-server-set-password
```
Сервис автоматически перезапустится после смены пароля.

## Просмотр логов

```bash
# Все логи (последние 50 строк)
user-service-logs

# Логи Code Server (100 строк)
user-service-logs code-server 100

# Логи Nginx
user-service-logs nginx

# Поиск в логах
journalctl --user --grep="error"
```

## Пересоздание конфигураций

Если возникли проблемы с сервисами:

```bash
# Запросить пересоздание конфигов
user-service-recreate-configs
```

## SSH доступ и проброс портов

### Подключение к серверу:

```bash
ssh ваш_пользователь@адрес_сервера
```

### Проброс портов на локальную машину:

```bash
# Проброс порта Code Server
ssh -L 8080:localhost:ВАШ_ПОРТ ваш_пользователь@адрес_сервера
```
После этого Code Server будет доступен локально:
- Локально: `http://localhost:8080`
- На сервере: `http://сeрвер:ВАШ_ПОРТ`

### Постоянный проброс (в фоновом режиме):

```bash
# Создать туннель в фоне
ssh -N -L 8080:localhost:ВАШ_ПОРТ ваш_логин@сервер &
# или с авто-переподключением
autossh -M 0 -N -L 8080:localhost:ВАШ_ПОРТ ваш_логин@сервер &
```

### Узнать ваш порт:

На сервере выполните:
```bash
# Способ 1: Через скрипт управления
user-service-control status | grep "HTTP Proxy Port"

# Способ 2: Из файла (если доступно)
cat /etc/user-services/ports.db | grep "^$(id -u):" | cut -d: -f2

# Способ 3: Через systemd (если сервис запущен)
systemctl --user show nginx-proxy.service | grep Listen | head -1
```

## Автоматическое управление

### Особенности работы:

- Code Server запускается автоматически при первом обращении 
- Code Server останавливается через 1 час без активности 
- Nginx работает постоянно для приема запросов 
- Все сервисы перезапускаются при сбоях

### При отсутствии активности:

- Закрыли браузер → сервис продолжает работать 1 час 
- Через 1 час → сервис автоматически останавливается 
- Новый запрос → сервис автоматически запускается

## Устранение неполадок

### Сервисы не запускаются:

```bash
# Проверить статус
user-service-control status

# Посмотреть ошибки
user-service-logs all 100

# Перезагрузить конфигурации
systemctl --user daemon-reload
```

### Нет доступа к веб-интерфейсу:

- Проверьте что сервисы запущены: user-service-control status 
- Проверьте что порт правильный (см. выше)
- Проверьте пароль: cat ~/.code-server-initial-password.txt 
- Попробуйте локальный проброс портов через SSH

### Systemd --user недоступен:

```bash
# Проверить linger
loginctl show-user $(whoami) | grep Linger

# Обычное решение - перелогин
logout
# и снова login
```

## Поддержка

Если проблемы не решаются:

- Сообщите администратору ваш UID: id -u 
- Предоставьте логи ошибок: user-service-logs all 100 
- Укажите шаги для воспроизведения проблемы

## Дополнительные возможности

### Ручное управление через systemd:

```bash
# Просмотр всех юнитов
systemctl --user list-units

# Детальная информация о сервисе
systemctl --user status code-server.service

# Просмотр логов конкретного сервиса
journalctl --user -u code-server.service -f
```

### Мониторинг активности:

```bash
# Активные соединения
ss -xp | grep code-server

# Процессы code-server
ps aux | grep code-server | grep -v grep
```
