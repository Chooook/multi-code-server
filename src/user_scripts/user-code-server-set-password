...
SERVICE_NAME="code-server.service"
...
current_hash=$(grep '^password:' "$CONFIG_FILE" | awk '{print $2}' | tr -d '[:space:]')
...
if systemctl --user is-active --quiet "$SERVICE_NAME"; then
    ...
    if command -v getent >/dev/null && [ -f "/etc/user-services/ports.db" ]; then
        ...
        if [ -n "$port_line" ]; then
            nginx_port=$(echo "$port_line" | cut -d: -f2)
            echo ""
            echo "Connection info:"
            echo "- URL: http://$(hostname -f 2>/dev/null || hostname):$nginx_port"
            echo "- Or: http://$(hostname -I | awk '{print $1}' 2>/dev/null):$nginx_port"
        fi
    fi
else
